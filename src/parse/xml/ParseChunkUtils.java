...
public class ParseChunkUtils {
	...
	public static void parseXmlHeader(byte[] byteSrc){
		...
		System.out.println("parse.xml size:"+Utils.bytesToHexString(xmlSize));

		xmlSb.append("<?parse.xml version=\"1.0\" encoding=\"utf-8\"?>");
		xmlSb.append("\n");
	}
	...
	public static void parseXmlContent(byte[] byteSrc){
		...
		System.out.println("parse parse.xml:\n"+xmlSb.toString());
	...
	public static void writeFormatXmlToFile(){
		FileWriter fw = null;
		try{
			fw = new FileWriter("xmltest/AndroidManifest_format.parse.xml");
			fw.write(xmlSb.toString());
		}catch(Exception e){
			System.out.println("write format parse.xml file error:"+e.toString());
		}finally{
			try{
				fw.close();
			}catch(Exception e){
